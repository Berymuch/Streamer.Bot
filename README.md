...
> [!NOTE]
> Botymuch v1.6.8 is a major release that changes how redeems are handled on a per-user basis. This will result in the duplication of some functions if updating from a previous version, and also means that both my latest VNyan and Streamer Bot logic are required in order for my logic to function as intended. Botymuch requires the latest stable version of Streamer.bot.

Command Documentation | Action Documentation | OBS Documentation | SFX Files | Clip Player | StreamDeck Profile
:-: | :-: | :-: | :-: | :-: | :-:
[Commands List](https://github.com/Berymuch/Streamer.Bot/blob/main/Chat%20Commands%20List.md) | [Actions List](https://github.com/Berymuch/Streamer.Bot/blob/main/Actions%20List.md) | [OBS Scene/Source List](https://github.com/Berymuch/Streamer.Bot/blob/main/Botymuch%20OBS%20source%20setup.md) | [SFX Files](https://github.com/Berymuch/Streamer.Bot/blob/main/Streamerbot%20SFX.zip) | [Clip Player](https://github.com/Berymuch/Streamer.Bot/blob/main/shoutouthd.html) | [StreamDeck Profile](https://github.com/Berymuch/Streamer.Bot/blob/main/Botymuch%20Streaming%20Layout.streamDeckProfile)

>*THIS BOT FEATURES COMPLETE TWITCH API INTEGRATION AND ACTION/COMMAND DOCUMENTATION. Youtube/Kick integrations are in development. This is a collection of hundreds of actions/commands I've created and collected to support my Twitch channel! It automates various components that I had previously relied on webserver-based bots for, and does so while running entirely locally from your computer.*
...
Latest version:
[Botymuch v1.6.9.sb](https://github.com/Berymuch/Streamer.Bot/blob/main/Botymuch%20v1.6.9.sb)
***

*REQUIRED:*

0. **Review the provided documentation to see what will be preconfigured when you install any of the actions in this bot as well as things that will need further configuration.** Links to documentation for commands, actions, and OBS source layout can be found near the top of this page.
...
6.  Certain functions of the bot are set to look towards specifically named OBS sources. In order to use them, you must first connect your OBS websocket to streamerbot then [create sources of the type and name provided in the source setup file above](https://github.com/Berymuch/Streamer.Bot/blob/main/Botymuch%20OBS%20source%20setup.md). A guide on how to connect via the OBS websocket can be found here: https://www.youtube.com/watch?v=u4GWb6NaNsw

7. If you don't already have a local clip shoutout player, download the "shoutouthd.html" file above and follow the setup instructions in "CONFIG ME (OPTIONAL)". You will need to add a new Scene to OBS, and inside of it create a new Browser source. In the browser source properties, Ensure the browser source URL is set to "about:blank" and that "control audio via OBS", "shutdown source when not visible", and "refresh browser when scene becomes active" are all checked. For Page Permissions, ensure that read access to user information (current Scene Collection, Transitions) is selected. Credit for this implementation goes out to my buddy [Liminality](https://www.twitch.tv/liminality)!
...
>*Note: some actions rely on specific scenes/filters/transitions/sources to be present in OBS to function- these aspects of this bot will not work out of the box but can be modified to suit your needs. Eventually a method will be provided to autopopulate OBS sources once the bot progresses further along the developmental roadmap. Actions marked as EX (excluded from the action history) are done so to avoid spamming the streamerbot action logs when run.*

All credit for the streamer bot application goes to its [developers](https://streamer.bot/).
