...
𝘼𝙋𝙄𝙓 Stars | 🟥 | 🟩 | Allows for the implementation of a chat message gacha system! The system is fully customizable when setup using the directions provided in the action, and gives a chance for every chat message to allow a user to earn a currency that can be used however is desired. Allows users to also check the total amount of currency they have, give some to others, wager it with various other commands, and have it incremented and decremented by mods. | [ItzApix_](https://www.twitch.tv/ItzApix_) & [Berymuch](https://www.twitch.tv/Berymuch)
