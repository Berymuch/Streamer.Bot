...
Pyramid Core | 🟩 | 🟩 | Controls subactions that are run when an emote pyramid is successful or broken. Also triggers when the !pyramid command is used and returns information based on the input parameters. Will also increment stars amount for the person who successfully builds a pyramid by 10 * the pyramid width and will increment stars by 50 for whoever destroys a pyramid. |  [ItzApix_](https://www.twitch.tv/ItzApix_) & [Berymuch](https://www.twitch.tv/Berymuch)
...
Raid | 🟩 | 🟩 | Controls subactions that are run when a Raid is received, sent, started, and cancelled. Also controls how the raid message is set and saved via the !raidmessage command. Will also increment stars amount for the person raiding by 10 * the amount of people in the raid. | [Berymuch](https://www.twitch.tv/Berymuch)
...
Watch Streak Core | 🟩 | 🟩 | Controls subactions that are run when a watchstreak is shared. Also triggers when the !watchstreak command is used and returns information based on the input parameters. Will also increment stars amount for the person sharing their wwatchstreak by 10 * the watchstreak amount. | [ItzApix_](https://www.twitch.tv/ItzApix_) & [Berymuch](https://www.twitch.tv/Berymuch)
