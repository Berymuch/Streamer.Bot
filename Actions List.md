...
Action | GFX | SFX | Description | Credit
:--- | :---: | :---: | :--- | :---:
Counter Increase Commands | 🟩 | 🟩 | Increments a global variable that stores the amount of times the streamer has burped or become distracted while live. It includes features which activate GFX/SFX effects and posts a confirmation message to chat. Also allows you to check the total for either counter by using the appropriate commands. | [Berymuch](https://www.twitch.tv/Berymuch)
...
Quick Math | 🟥 | 🟥 | This action powers the interactive "quick maths" chat game. Users are able to use the command to return a random or tiered math problem, and then have a set amount of time to answer it in chat. The logic also interacts with the chat currency system to give winners leveled rewards and antes can even be offered up as an incentive/bet if the user of the command is confident that they will be able to answer it first. | [Berymuch](https://www.twitch.tv/Berymuch)
Quotes | 🟥 | 🟥 | This action consolidates all four major quote interactions into one switch tree. Using this logic, users are able to leverage the built in quotes system provided by streamerbot to add, delete, list, and display random or specific quotes that have been stored. It also supports quotes browsing by using search terms after the command, and attempts to return the closest matching Quote. Editing quotes is more easily done through the built in quote manager and thus a command to do so has not been implemented. | [Berymuch](https://www.twitch.tv/Berymuch) & [ItzApix_](https://www.twitch.tv/ItzApix_)
Random Bot Chat Messages | 🟩 | 🟥 | Randomly selects and sends a "tip" chat message that showcases an aspect of the stream. Frequency by default is once every 20 minutes and if 75 messages have been sent (both have to be true) in chat since the last randomized message was run. Numbering is handled by the random number generator and an argument set to match the total amount of tips. | [Berymuch](https://www.twitch.tv/Berymuch)
...
Landmine | 🟩 | 🟩 | Used to plant a chat landmine! Anyone who steps on it with a chat message will trigger it and time themselves out for 15s while also losing 10% of their chat currency. If the deactivation code is guessed correctly (chosen randomly between 1-100), the person who defuses the landmine receives +10% of their total chat currency! Can also plant super landmines which timeout multiple users randomly when triggered. Mods are unable to trigger or be timed out by the mine. Moderators can plant soft or hard landmines- hard ones time users out while soft landmines only surprise! | [Berymuch](https://www.twitch.tv/Berymuch) & [ItzApix_](https://www.twitch.tv/ItzApix_)
Multi Shoutout | 🟥 | 🟥 | Allows you to queue up to as many usernames desired to be shouted out sequentially via standard Twitch shoutouts, and posts a link in chat for each channel. Shoutouts are sent 2 minutes apart to align with Twitch functions, and intelligently check to ensure users actually exist and that there are no typographical errors in the names. | [ItzApix_](https://www.twitch.tv/ItzApix_) & [Berymuch](https://www.twitch.tv/Berymuch)
Spammer Jail | 🟥 | 🟥 | Allows you to add/remove individual users from the Spammer Jail user group to enable/disable the usage of all chat commands and redeems. | [Berymuch](https://www.twitch.tv/Berymuch)
...
Action | GFX | SFX | Description | Credit
:--- | :---: | :---: | :--- | :---:
CTRL-Z Tracker | 🟩 | 🟥 | Tracks the amount of times the streamer presses CTRL-Z! Stores both a lifetime total, session total, session highscore, and allows usage of the associated command to send the values for both to Twitch chat. | [Berymuch](https://www.twitch.tv/Berymuch)
...
Chat | 🟩 | 🟩 | Controls subactions that occur when cheers are recieved. Also triggers when teaminfo, accountage, and followage commands are used, bits badge tiers are updated, when a user types their first chat message in a 12hr period, and when detecting first/second/third chatters of the stream and recognizing them accordingly. Controls walkon SFX triggers and also has logic to facilitate celebrating modiversaries and the pinning/unpinning of messages in Twitch chat.| [Berymuch](https://www.twitch.tv/Berymuch)
...
Kofi, StreamLabs, & StreamElements | 🟩 | 🟩 | Controls subactions that are run when events from KoFi/Streamlabs/StreamElements are received. Requires additional setup with each respective service as specified in the action comments. | [Berymuch](https://www.twitch.tv/Berymuch)
...
Summarizer Core | 🟥 | 🟥 | Used to generate an end of stream summary to help you recap. | [ItzApix_](https://www.twitch.tv/ItzApix_) & [Berymuch](https://www.twitch.tv/Berymuch)
Watch Streak Core | 🟩 | 🟩 | Controls subactions that are run when a watchstreak is shared. Also triggers when the !watchstreak command is used and returns information based on the input parameters. | [ItzApix_](https://www.twitch.tv/ItzApix_), [Berymuch](https://www.twitch.tv/Berymuch), & [osuphoenix](https://extensions.streamer.bot/t/top-watch-streaker/1870)
