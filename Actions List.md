...
Chat Games | 🟥 | 🟥 | This action powers the interactive "quick maths" and gamba chat games. Users are able to use the command to return a random or tiered math problem, and then have a set amount of time to answer it in chat. Antes can even be offered up as an incentive/bet if the user of the command is confident that tthey will be able to answer it first. The Gamba command allows them to gamble their chat currency in various ways.  | [Berymuch](https://www.twitch.tv/Berymuch)
