...
Pyramid Core | 🟩 | 🟩 | Controls subactions that are run when an emote pyramid is successful or broken. Also triggers when the !pyramid command is used and returns information based on the input parameters. Will also increment stars amount for the person who successfully builds a pyramid by 10 * the pyramid width. |  [ItzApix_](https://www.twitch.tv/ItzApix_) & [Berymuch](https://www.twitch.tv/Berymuch)
